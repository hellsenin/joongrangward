...
public class EgovSysInfo {
	...
	*/
	public static String getPrductVersion(String server) throws Exception {
		...
		String version = EgovProperties.getProperty(Globals.ServerConfPath, server.toUpperCase() + ".VERSION");
		return version;
	}
	...
	*/
	public static String getPrductPort(String server) throws Exception {
		...
		String port = EgovProperties.getProperty(Globals.ServerConfPath, server.toUpperCase() + ".PORT");
		return port;
	}
	...
	*/
	public static String getPrductStatus(String port) throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.ServerConfPath, "SHELL."+Globals.OsType+".getPrductStatus");
	        ...
	        } else if ("WINDOWS".equals(Globals.OsType)) {
	        	while (true){
	        	    String str = b_out.readLine();
	        	    if (str == null) break;
	...
	*/
	public static String getOSName() throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.ServerConfPath, "SHELL."+Globals.OsType+".getOSInfo");
            String[] command = {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
            					"NAME"};
            p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
	...
	*/
	public static ArrayList getDiskName() throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.ServerConfPath, "SHELL."+Globals.OsType+".getDiskInfo");
            String[] command = {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
            					"NAME"};
            p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
	...
	*/
	public static float getMoryFullCpcty() throws Exception {
		...
		try
		{   
			String cmdStr = EgovProperties.getProperty(Globals.ServerConfPath, "SHELL."+Globals.OsType+".getMoryInfo");
            String[] command = {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
            					"FULL"};
            p = Runtime.getRuntime().exec(command);
		    //p.waitFor();
    ...
    */
    public static ArrayList executeProgram(String propertyKeyword, String[] command) throws Exception {
        ...
        {       
            Process p = null;
            String cmdStr = EgovProperties.getProperty(Globals.ShellFilePath, "SHELL."+Globals.OsType+"."+propertyKeyword);
            command[0]=cmdStr;
            p = Runtime.getRuntime().exec(command);
            p.waitFor();
            //프로세스 에러시 종료
	...
	public static ArrayList getProcessId() {
		...
		try {
			...
			if ("WINDOWS".equals(Globals.OsType)) {
				...
				separator = "\",\"";
			} else if ("UNIX".equals(Globals.OsType)) {
				/*
    ...
    public static ArrayList getProcessId(String processName) {
		...
		try {
			...
			Process p = null;
			//log.debug("Globals.OsType:"+Globals.OsType);
			if ("WINDOWS".equals(Globals.OsType)) {
