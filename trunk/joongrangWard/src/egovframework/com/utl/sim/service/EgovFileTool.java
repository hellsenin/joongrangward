...
public class EgovFileTool {
    ...
    public static ArrayList getSubFilesByOwner(File[] fileArray, String owner) throws Exception {
        ...
        for (int i = 0; i < fileArray.length; i++) {
            ...
            // 파일이면, 생성자가 같은지 비교한다.
                ...
                Process p = null;
                if(Globals.OsType.equals("UNIX")){
                    String[] command = {EgovProperties.getProperty(Globals.ShellFilePath, "SHELL."+Globals.OsType+".getDrctryByOwner"),
                                        fullpath.substring(0,fullpath.lastIndexOf("/")),
                                        fullpath.substring(fullpath.lastIndexOf("/"),fullpath.length()),
                                        owner};
                    p = Runtime.getRuntime().exec(command);
                    p.waitFor();
                }else if(Globals.OsType.equals("WINDOWS")){
                    String command = EgovProperties.getProperty(Globals.ShellFilePath, "SHELL."+Globals.OsType+".getDrctryByOwner");
                    p = Runtime.getRuntime().exec(command);
                    p.waitFor();
                } 
        ...
        try {
            ...
            if (file.exists() && file.isFile()) {
                ...
                for (int i = 0; i < strArr.length; i++) {
                    ...
                    int idx = 0;
                    boolean result = false;
                    for (int j = 0; j < parLen.length && result != true; j++) {
                    	String split = "";
    ...
    public static boolean cmprFilesByContent(String cmprFile1, String cmprFile2) throws Exception {
        ...
        try {
            ...
            if (file1.exists() && file2.exists()
                ...
                for (int i = 0; i < cmprText1.size() && isWrong != true; i++) {
    ...
    */
    public static boolean copyFiles(String [] source, String target) throws Exception {
        ...
        // 복사를 시작한다.
        for (int j = 0; j < source.length && result != false; j++) {
    ...
    */
    public static boolean copyFilesByExtnt(String source, String extnt, String target) throws Exception {
        ...
        if (srcFile.exists() && srcFile.isDirectory()) {
            ...
            // 복사를 시작한다.
            for (int i = 0; i < list.size() && result != false; i++) {
    ...
    public static String getMountLc(String file) throws Exception {
        ...
        {   
            ...
            if (srcFile.exists()) {
                ...
                if(Globals.OsType.equals("UNIX")){
                    Process p = null;
                    String[] command = {EgovProperties.getProperty(Globals.ShellFilePath, "SHELL."+Globals.OsType+".getMountLc"), src, "/"};
                    p = Runtime.getRuntime().exec(command);
                    //p.waitFor();
    ...
    public static String getOwner(String file) throws Exception {
        ...
        {   
            ...
            if (srcFile.exists()) {
                ...
                Process p = null;
                String cmdStr = EgovProperties.getProperty(Globals.ShellFilePath, "SHELL."+Globals.OsType+".getDrctryOwner");
    ...
    public static String getAccess(String file) throws Exception {
        ...
        {   
            ...
            if (srcFile.exists()) {
                ...
                Process p = null;
                if(Globals.OsType.equals("UNIX")){
                    String[] command = {EgovProperties.getProperty(Globals.ShellFilePath, "SHELL."+Globals.OsType+".getDrctryAccess"),
                                        parentPath, 
                                        fname};
                    p = Runtime.getRuntime().exec(command);
                    p.waitFor();
        ...
        {   
            ...
            if (srcFile.exists()) {
                ...
                if (strArr.length >= 2) {
                    ...
                    type = EgovProperties.getProperty(Globals.FileFormatPath, format);
                }
    ...
    */
        ...
        {   
            ...
            if(f.exists()){
                ...
                if(targetDir.exists()){
                    ...
                    String cmdStr = EgovProperties.getProperty(Globals.ShellFilePath, "SHELL."+Globals.OsType+".moveDrctry");
                	String[] command = {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR),
    ...
    public static boolean startDirectoryMonitering(String targetDirPath) throws Exception {
        ...
        try{
        	...
        	File logF = new File(Globals.ConfPath+"/"+targetF.getName()+".log");
