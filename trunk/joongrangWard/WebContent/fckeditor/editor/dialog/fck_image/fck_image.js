...
function Ok()
{
	...
	{
		if ( confirm( '선택하신 이미지를 이미지 버튼으로 바꾸시겠습니까?' ) )
			oImage = null ;
	}
	else if ( bHasImage && !bImageButton && oImage.tagName == 'INPUT' )
	{
		if ( confirm( '선택하신 이미지 버튼을 이미지로 바꾸시겠습니까?' ) )
			oImage = null ;
	}
...
function OnUploadCompleted( errorNumber, fileUrl, fileName, customMsg )
{
	...
	switch ( errorNumber )
	{
		...
		case 101 :	// Custom warning
			...
			break ;
		case 201 :
			alert( '같은 이름의 파일이 존재합니다. "' + fileName + '"으로 파일명이 변경되었습니다.' ) ;
			break ;
		case 202 :
			alert( '파일형식이 올바르지 않습니다.' ) ;
			return ;
		case 203 :
			alert( "보안 문제입니다. 업로드 할 수 있는 권한이 없습니다. 서버관리자에게 문의하십시오." ) ;
			return ;
		case 500 :
			alert( '연결 문제입니다. 업로드 할 수 있는 권한이 없습니다. 서버관리자에게 문의하십시오.' ) ;
			break ;
		default :
			alert( '파일업로드 에러. 에러번호: ' + errorNumber ) ;
			return ;
	}
