...
<script type="text/javascript">
	...
	{	
		...
		var param="&onlineId=${result.onlineId}&applyCode=${param.applyCode}&inwon="+$F("inwon")+"&degree="+$F("degree");
	...
	function callbackLottery(oj)
	{
		try 
		{
			...
			{	
				 alert("성공적으로 수정되었습니다.");
				 location.href = "/admin/health/onlineApply/selectApplicantList.do?applyCode=${param.applyCode}&onlineId=${result.onlineId}&status=2";
			}
			else if(code == "0")
			{
				 alert("알수없는 오류입니다.");
	...
	{
		location.href="/admin/health/onlineApply/selectApplicantList.do?onlineId=${result.onlineId}&amp;applyCode=${param.applyCode}&status="+status;
	}
				...
				<h3 class="tit_03">추첨하기</h3>
					<table class="default_view">
						...
						<tr>
							...
							<select name="degree" title="구분항목 선택" class="vam" style="width: 100px; float: margin-top: 2px;" >
								<option value="">선택하세요</option>
								<option value="1">선착순추첨</option>
								<option value="2">무작위추첨</option>
							</select>
							</td>
						</tr>
						<tr>
