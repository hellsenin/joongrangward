...
<table summary="알림 관리 페이지" class="default_view">
		...
		<tr>
			...
			<td class="output">
				<c:choose>
					...
					<c:when test="${result.category == '88'}">보건소</c:when>
...
<div class="board_btn_set mt13">
		...
		<c:url value="/admin/health/portalAlert/delete.do" var="del_url">
		...
		<c:url value="/admin/health/portalAlert/forUpdate.do" var="upt_url">
			<c:param name="alert_cd" value="${result.alert_cd}"/>
			<c:param name="pageIndex" value="${Bean.pageIndex}"/>
			<c:param name="searchCondition" value="${Bean.searchCondition}"></c:param>
			<c:param name="category" value="${Bean.category}"></c:param>
