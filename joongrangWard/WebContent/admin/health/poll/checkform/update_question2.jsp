...
<br/>
	...
	<form name="fm" action="<c:url value="/admin/health/poll/checkform/selectQuestion2ForUpdate.do"/>" method="post" onsubmit="return checkAndSubmit(this);">
...
<div class="board_btn_set mt13">
			<c:choose>
				<c:when test="${Bean.view_state != 'select'}">
						<c:url var="cancel_action" value="/admin/health/poll/checkform/selectQuestion2ForUpdate.do">
	...
	<span class="btn_list"><a href="${cancel_action}">취소보기</a></span>
	...
	<span class="btn_del"><input type="submit" value="수정하기" onclick="modify();"/></span>
						...
						<c:url var="list_action" value="/admin/health/poll/checkform/list_question.do">
							<c:param name="searchKeyword" value="${Bean.searchKeyword}"/>
							<c:param name="year_cd" value="${Bean.year_cd}"/>
							<c:param name="master_cd" value="${Bean.master_cd}"/>
						</c:url>
