...
function fncSelectNonRegistedDeptAuthorList(pageNo) {
	document.listForm.deptCode.value = '';
	document.listForm.deptNm.value = '';
    document.listForm.searchCondition.value = "1";
    document.listForm.pageIndex.value = pageNo;
    document.listForm.action = "<c:url value='/cmm/sec/ram/EgovNonRegistedDeptAuthorList.do'/>";
    document.listForm.submit();
}
...
function fncSelectDeptAuthorPop() {
    ...
    //var url = "<c:url value='/cmm/sec/ram/EgovDeptSearchList.do'/>";
    //var openParam = "dialogWidth:500px;dialogHeight:485px;scroll:no;status:no;center:yes;resizable:yes;";
    /*
    var retVal = window.showModalDialog(url, varParam, openParam);
    if(retVal) {
    ...
    */
	...
	if(retVal) {
        document.listForm.deptCode.value = retVal.deptId;
        document.listForm.deptNm.value = retVal.deptNm;
    }
...
function setAuthoValue(deptCode, deptNm)
{
	listForm.deptCode.value = deptCode;
	listForm.deptNm.value = deptNm;
}
-->
</script>
...
<form name="listForm" action="<c:url value='/cmm/sec/ram/EgovDeptAuthorList.do'/>" method="post">
	<input type="hidden" name="mberTyCode" value="A" />
<table width="100%" cellpadding="8" class="table-search" border="0">
 <tr>
  ...
  <td width="55%">부서<img src="/images/egovframework/uss/olp/mgt/icon/required.gif" width="15" height="15"> : <input name="deptCode" type="text" value="<c:out value="${deptAuthorVO.deptCode}"/>" size="12" title="부서코드" onkeypress="press();" readOnly />
                              <input name="deptNm" type="text" value="<c:out value="${deptAuthorVO.deptNm}"/>" size="15" title="부서명" onkeypress="press();" readOnly />
  </td>
  <td width="4%">
  ...
  <th width="17%" align="right">
   <table border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td><img src="<c:url value='/images/egovframework/cmm/sec/ram/btn/bu2_left.gif' />" width="8" height="20"></td>
      <td background="<c:url value='/images/egovframework/cmm/sec/ram/btn/bu2_bg.gif'/>" class="text_left" nowrap><a href="javascript:fncSelectNonRegistedDeptAuthorList('1')" style="selector-dummy:expression(this.hideFocus=false);">미등록사용자조회</a> 
      </td>
      <td><img src="<c:url value='/images/egovframework/cmm/sec/ram/btn/bu2_right.gif'/>" width="8" height="20"></td>
      <td>&nbsp;&nbsp;</td>
      <td><img src="<c:url value='/images/egovframework/cmm/sec/ram/btn/bu2_left.gif' />" width="8" height="20"></td>
...
<table width="100%" cellpadding="8" class="table-line">
 ...
 <c:forEach var="deptAuthor" items="${deptAuthorList}" varStatus="status">
  <tr>
    <td class="lt_text3" nowrap><input type="checkbox" name="delYn" class="check2"><input type="hidden" name="checkId" value="<c:out value="${deptAuthor.userId}"/>" /></td>
