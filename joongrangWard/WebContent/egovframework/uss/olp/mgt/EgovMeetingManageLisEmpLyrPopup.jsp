...
function fn_egov_search_MeetingManage(){
	...
	vFrom.action = "/uss/olp/mgt/EgovMeetingManageLisEmpLyrPopup.do";
	vFrom.submit();
...
function fn_egov_open_Popup(cnt, uniqId){
	...
	*/
	if(opener[1] == "typeMeeting"){
		opener[0].document.getElementById("mnaerId").value = uniqId;
		...
		opener[0].document.getElementById("schdulChargerId").value = uniqId;
...
<table width="100%" cellpadding="1" class="table-search" border="0">
 <tr>
  <td class="title_left">
   ...
   <img src="<c:out value="${ImgUrl}"/>icon/tit_icon.gif" width="16" height="16" hspace="3" align="absmiddle">&nbsp;아이디 검색</td>
  <th>
  </th>
  <td width="110px">
...
<tr>  
    ...
    <th class="title" width="100px" nowrap>아이디</th>
    <th class="title" width="100px"  nowrap>이름</th>
    <th class="title" width="100px" nowrap>전화번호</th>
...
<tr>
		...
		<td class="lt_text3L">${resultInfo.emplyrNm}</td>
		<td class="lt_text3">${resultInfo.offmTelno}</td>
		<td class="lt_text3L">${resultInfo.homeadres} ${resultInfo.detailAdres}</td>
		
    	<td class="lt_text3L"><a href="#" onClick="JavaScript:fn_egov_open_Popup('${status.count}', '${resultInfo.uniqId}')">선택</td>
