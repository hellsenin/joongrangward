...
<div class="board_top clearfix">
    ...
    <fieldset class="fl_r">
        ...
        <select name="searchCnd" id="searchCnd">
			...
			<option value="NTT_SJ" <c:if test="${searchVO.searchCnd == 'NTT_SJ'}">selected="selected"</c:if> >약국명</option>
			<option value="OPTION1" <c:if test="${searchVO.searchCnd == 'OPTION1'}">selected="selected"</c:if> >소재지</option>
        </select>
		<label for="searchWrd" class="blind">구분선택</label>
        <input type="text" title="검색어" class="text" name="searchWrd" id="searchWrd" value='<c:out value="${searchVO.searchWrd}"/>' />
    <input type="image" src="/health/open_content/images/borad/btn_search.gif" alt="검색" />
    </fieldset>
</div>
</form>

<table class="default_list" summary="휴일야간진료 게시판으로 번호, 약국명, 소재지, 전화번호, 지도로 구성">
    <caption>휴일야간진료 게시판</caption>
    <thead>
        <tr>
            <th width="10%" scope="col">번호</th>
            <th width="25%" scope="col">약국명</th>
            <th width="35%" scope="col">소재지</th>
    ...
    <tbody>
        ...
        <tr>
			...
			<td><a href="#ready" onclick="alert('준비중입니다');">지도</a></td>
        </tr>
